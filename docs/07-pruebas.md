...
| `contenedorService.test.js` | Transiciones de estado del contenedor, cálculo de costes D&D con múltiples tramos tarifarios, manejo de días libres, y la validación del formato del código BIC al editar (4 letras + 7 números → 422; normalización a mayúsculas). El reloj del sistema se fija con `jest.useFakeTimers` y se ajusta por test en los casos que verifican cálculos de días exactos. |
...
### 2.5 Pruebas de componentes — frontend (12 archivos)

Cubren tanto átomos como moléculas, verificando renderizado condicional, accesibilidad e interacciones del usuario con `userEvent`. Además de los cuatro componentes documentados en detalle a continuación, se prueban `BotonEditar`, `BotonEliminar`, `CeldaTabla`, `EstadoContenedorSemaforo`, `RolAsignado` (átomos) y `CardContenedor`, `ModalConfirmacion`, `ModalEditarContenedor` (moléculas), comprobando en cada uno sus estados, las clases de modificador aplicadas y los callbacks que disparan. `CardContenedor` verifica además que el botón de eliminar sale deshabilitado cuando el contenedor tiene un ciclo activo, y `ModalEditarContenedor` que solo permite guardar un código BIC con formato válido (4 letras + 7 números).
...
Resultados obtenidos ejecutando `npm test -- --coverage` sobre la suite completa (**261 tests, 24 suites**):

```
--------------------------|---------|----------|---------|---------|-----------------------
Archivo                   | % Stmts | % Branch | % Funcs | % Lines | Líneas no cubiertas
--------------------------|---------|----------|---------|---------|-----------------------
All files                 |   83.39 |    68.24 |    87.6 |   85.38 |
 ...
 services                 |   75.95 |    67.81 |   81.53 |   78.77 |
  ...
  contenedorService.js    |   58.91 |       50 |   81.81 |   62.56 | 28-30,85-97,203-260,457-482
...
| **Middlewares** | 80 % | 57.69 % | 100 % | 81.08 % |
| **Servicios** | 75.95 % | 67.81 % | 81.53 % | 78.77 % |
| **Total** | **83.39 %** | **68.24 %** | **87.6 %** | **85.38 %** |
...
Resultados obtenidos ejecutando `npm run test:coverage` sobre la suite unitaria/de componentes (**122 tests, 15 suites**; los 20 tests E2E de Playwright se ejecutan aparte con `npm run test:e2e`):

```
--------------------------|---------|----------|---------|---------|-------------------
File                      | % Stmts | % Branch | % Funcs | % Lines | Uncovered Line #s
--------------------------|---------|----------|---------|---------|-------------------
All files                 |   88.88 |    83.63 |   91.66 |   90.14 |
 components/atomos         |   93.33 |    90.52 |   94.73 |   94.87 |
 ...
 components/moleculas      |   77.19 |       75 |      75 |   78.43 |
  CardContenedor.jsx       |   81.81 |    91.66 |     100 |   81.81 | 16-17
  ModalConfirmacion.jsx    |     100 |    83.33 |     100 |     100 | 38-50
  ModalEditarContenedor.jsx|   67.64 |     62.5 |      50 |   71.87 | 36-42,62,95,122
...
| **Hooks** | 100 % | 75 % | 100 % | 100 % |
| **Componentes — átomos** | 93.33 % | 90.52 % | 94.73 % | 94.87 % |
| **Componentes — moléculas** | 77.19 % | 75 % | 75 % | 78.43 % |
| **Total** | **88.88 %** | **83.63 %** | **91.66 %** | **90.14 %** |

El total baja respecto a versiones anteriores porque `ModalEditarContenedor.jsx` pasa a contar en la cobertura al añadirse su test (67.64 %); sus líneas no cubiertas son ramas de carga de foto y de error de red. El resto de ramas sin cubrir son props opcionales de presentación (`Input`, `CeldaTabla`, `CardContenedor`) y una rama de error de `session.js`.

---

## 6. Resultados

**Backend:** todos los tests pasan en el entorno de CI de GitHub Actions. El workflow `ci.yml` ejecuta `npm test` en el job `test-backend` en cada push o Pull Request a `main` o `dev`. El número total de tests es **261** distribuidos en **24 suites** (10 de controladores + 10 de servicios + 2 de middlewares + 2 de integración), con cero fallos en la ejecución final del proyecto.

**Frontend:** la suite unitaria y de componentes suma **122 tests** en **15 suites** (12 de componentes + 2 de hooks + 1 de servicios), ejecutada con Vitest. A ello se añaden **20 tests end-to-end** con Playwright repartidos en **3 specs**, que cubren los flujos de login, almacén y semáforo en un navegador real.

**Total combinado: 403 tests (261 backend + 122 frontend unitarios/componentes + 20 frontend E2E), 42 suites.**
