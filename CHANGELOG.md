...
- Contraste **AAA** (≥7:1) en todas las páginas del perfil gestor (semáforo, tarifas, almacén, historial, perfil), tras auditar las 184 combinaciones texto/fondo en claro y oscuro. Cambios: fondo de etiqueta de error a rojo más oscuro (`--color-error-fondo` 5.6→8.0:1 con texto blanco); `--color-error-text`/`--color-success-text` claros oscurecidos a AAA sobre fondos neutros; tramo amarillo oscuro del semáforo (`primer_tramo` oscuro) oscurecido para texto blanco AAA (5.78→8.42:1); nuevo token `--color-placeholder` (sin `opacity`, que erosionaba el contraste) para los placeholders de los buscadores; texto de la notificación y botón de confirmar borrado pasan a colores que cumplen AAA; el enlace del pie usa subrayado en hover en vez de `opacity`; los textos de estado del semáforo usan texto pleno; y los requisitos de contraseña del perfil se muestran sobre una caja neutra (el verde de «cumplido» no alcanza AAA sobre el azul de la tarjeta).
- Diferenciación de los **botones de acción sobre tarjetas de color** (almacén, tabla de tarifas, botón de editar fechas de cada ciclo del historial y botones de la tarjeta del perfil): usaban el mismo azul `secondary` que la tarjeta y se fundían con ella (1.05:1, sin borde perceptible). Ahora usan una rampa de azul claro basada en **`#A4D3F4`** (token `--color-btn-card`, con sus variantes hover/off para claro y oscuro) con texto oscuro: el mismo azul claro en ambos modos (sin invertir nada), con texto AAA (≥9.5:1) sobre el botón, un fuerte resalte sobre la tarjeta oscura (5.6:1) y una diferencia clara sobre la tarjeta clara. El botón de guardar del perfil se reestiliza solo en ese contexto (login/registro no cambian).
