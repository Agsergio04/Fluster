...
- Contraste de **elementos no textuales** (WCAG 1.4.11, 3:1): los botones de acción que van sobre tarjetas de color (`Ver registro`, `Borrar`, `Generar informe`, `Actualizar`/`Eliminar` de tarifas) usaban el mismo azul `secondary` que la tarjeta `card-almacen` y se fundían con ella (1.05:1, sin borde perceptible). Ahora usan un color de **polaridad invertida** (`--color-btn-card`): azul oscuro con texto blanco en tema claro y azul claro con texto oscuro en oscuro, con un borde a 3.95/4.01:1 frente a la tarjeta y texto AAA (9.15/9.27:1).
